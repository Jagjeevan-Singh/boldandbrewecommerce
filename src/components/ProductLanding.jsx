...
function ProductLanding({ products, onAddToCart, onAddToWishlist }) {
  ...
  // If product is in exclusion list, block rendering and show not found
  if (product && EXCLUDE_NAMES.includes(String(product.name || '').trim().toUpperCase())) {
    product = null;
  }
    ...
    <>
      ...
      <div className="product-landing">
        ...
        <div className="product-details">
          ...
          <div className="buttons" style={{display:'flex', gap:'0.6rem', marginLeft:'-10px', flexWrap:'wrap'}}>
            <button onClick={() => onAddToCart(effectiveProduct)} disabled={effectiveProduct.stock === 0} style={{padding: window.innerWidth > 900 ? '1.1rem 2.2rem' : window.innerWidth > 768 ? '0.9rem 1.8rem' : '0.8rem 1.6rem', border:'none', cursor:effectiveProduct.stock === 0 ? 'not-allowed' : 'pointer', borderRadius:'10px', fontWeight:'700', fontSize: window.innerWidth > 900 ? '0.9rem' : window.innerWidth > 768 ? '0.8rem' : '0.75rem', background:'linear-gradient(135deg, #d32f2f 0%, #bf360c 100%)', color:'white', transition:'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', opacity: effectiveProduct.stock === 0 ? 0.5 : 1, boxShadow: effectiveProduct.stock === 0 ? 'none' : '0 4px 15px rgba(191, 54, 12, 0.4)', transform: 'translateY(0)', letterSpacing:'0.5px', textTransform:'uppercase'}} onMouseEnter={(e) => {if(effectiveProduct.stock !== 0) e.target.style.transform = 'translateY(-2px)'; if(effectiveProduct.stock !== 0) e.target.style.boxShadow = '0 8px 25px rgba(191, 54, 12, 0.6)';}} onMouseLeave={(e) => {e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = effectiveProduct.stock === 0 ? 'none' : '0 4px 15px rgba(191, 54, 12, 0.4)';}}>🛒 Add to Cart</button>
            <button onClick={() => onAddToWishlist(effectiveProduct)} disabled={effectiveProduct.stock === 0} style={{padding: window.innerWidth > 900 ? '1.1rem 2.2rem' : window.innerWidth > 768 ? '0.9rem 1.8rem' : '0.8rem 1.6rem', border:'2px solid #bf360c', cursor:effectiveProduct.stock === 0 ? 'not-allowed' : 'pointer', borderRadius:'10px', fontWeight:'700', fontSize: window.innerWidth > 900 ? '0.9rem' : window.innerWidth > 768 ? '0.8rem' : '0.75rem', background:'transparent', color:'#bf360c', transition:'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', opacity: effectiveProduct.stock === 0 ? 0.5 : 1, boxShadow: 'none', transform: 'translateY(0)', letterSpacing:'0.5px', textTransform:'uppercase'}} onMouseEnter={(e) => {if(effectiveProduct.stock !== 0) e.target.style.background = 'rgba(191, 54, 12, 0.1)'; if(effectiveProduct.stock !== 0) e.target.style.transform = 'translateY(-2px)'; if(effectiveProduct.stock !== 0) e.target.style.boxShadow = '0 8px 25px rgba(191, 54, 12, 0.3)';}} onMouseLeave={(e) => {e.target.style.background = 'transparent'; e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = 'none';}}>❤️ Wishlist</button>
          </div>
